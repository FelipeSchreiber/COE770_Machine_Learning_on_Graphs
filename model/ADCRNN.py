...
class ADConv(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, K, bias=True):
        super(ADConv, self).__init__(aggr="add", flow="source_to_target")
        assert K > 0
    ...
    def __reset_parameters(self):
        ...
        #torch.nn.init.uniform_(self.residual_matrix, a=0.0, b=1.0)
...
class ADCRNN(torch.nn.Module):
    ...
    def forward(
        ...
        adj_mat = to_dense_adj(edge_index, edge_attr=edge_weight)
        adj_mat = adj_mat.reshape(adj_mat.size(1), adj_mat.size(2))
        self.residual_matrix = NN.Softmax(NN.PReLU(torch.mm(X, X.transpose(0, 1))))
